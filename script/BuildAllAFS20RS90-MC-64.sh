...
INPUT_SEQUENCES=Genomica/InputDatasets/AFS20RS90
...
spark-submit --class com.github.jmabuin.metacachespark.MetaCacheSpark --conf spark.network.timeout=10000000  --num-executors $PARTITION_NUMBER --master yarn --executor-memory 45G --driver-memory 20G MetaCacheSpark-0.3.0.jar -m build -d hashmultimap_native -w -e -l -p $PARTITION_NUMBER -t $TAXONOMY $DATABASE_NAME $INPUT_SEQUENCES
