...
class RunnableClassificationPaired implements Runnable {
    private Thread t;
    ...
    RunnableClassificationPaired(Query query_obj, Map<Integer, Classification> classifications, String f1, String f2, String name,
                                 int startRead, int bufferSize, int targetWindowStride,
                                 MetaCacheOptions options,
                                 List<Integer> data, List<Integer> data2, List<String> headers) {
    ...
    public void run() {
        ...
        try {
            ...
            hits = this.query_obj.db.accumulate_matches_paired(f1, f2,
            ...
            for(int i = 0;  i < hits.size() ; i++) {
                ...
                //classify ----------------------------------
                long numWindows = (2 + Math.max(data_len + data2_len, this.options.getProperties().getInsertSizeMax()) / this.targetWindowStride);
