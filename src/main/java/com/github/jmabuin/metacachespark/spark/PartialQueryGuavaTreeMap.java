 ...
 */
public class PartialQueryGuavaTreeMap implements PairFlatMapFunction<Iterator<HashMultimap<Integer, LocationBasic>>, Long, TreeMap<LocationBasic, Integer>> {

    private static final Log LOG = LogFactory.getLog(PartialQueryGuavaTreeMap.class);
    ...
    public PartialQueryGuavaTreeMap(String file_name, long init, int bufferSize, long total, long readed, int result_size) {
    ...
    @Override
    public Iterator<Tuple2<Long, TreeMap<LocationBasic, Integer>>> call(Iterator<HashMultimap<Integer, LocationBasic>> myHashMaps) {
                ...
                while((this.seqReader.next() != null) && (currentSequence < (this.init + this.bufferSize))) {
                    ...
                    for(Sketch currentSketch: locations) {
                        ...
                        for(int location: currentSketch.getFeatures()) {
                            ...
                            Set<LocationBasic> values = currentHashMap.get(location);
