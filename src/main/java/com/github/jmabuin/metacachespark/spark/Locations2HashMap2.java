...
public class Locations2HashMap2 implements FlatMapFunction<Iterator<Tuple2<Integer, LocationBasic>>, HashMap<Integer, List<LocationBasic>>> {

    private static final Log LOG = LogFactory.getLog(Locations2HashMap2.class);
    ...
    public Iterator<HashMap<Integer, List<LocationBasic>>> call(Iterator<Tuple2<Integer, LocationBasic>> inputLocations){
        ...
        while(inputLocations.hasNext()) {
            ...
            if (!map.containsKey(current._2().getTargetId())) {
                map.put(key, new ArrayList<LocationBasic>());
            }

            map.get(key).add(new LocationBasic(current._2().getTargetId(), current._2().getWindowId()));
