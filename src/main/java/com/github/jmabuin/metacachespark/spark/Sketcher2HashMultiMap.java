 ...
 */
public class Sketcher2HashMultiMap implements Function2<Integer, Iterator<Sequence> ,Iterator<HashMultimap<Integer,LocationBasic>>> {

	private static final Log LOG = LogFactory.getLog(Sketcher2HashMultiMap.class);
	private short k_;
	...
	@Override
	public Iterator<HashMultimap<Integer, LocationBasic>> call(Integer partitionId, Iterator<Sequence> inputSequences){
			...
			while (currentStart < (inputSequence.getData().length() - MCSConfiguration.kmerSize)) {
				...
				if ((currentEnd - currentStart) >= MCSConfiguration.kmerSize) {
					...
					for (int newValue : sketchValues) {
						...
						Set<LocationBasic> currentList = map.get(newValue);
