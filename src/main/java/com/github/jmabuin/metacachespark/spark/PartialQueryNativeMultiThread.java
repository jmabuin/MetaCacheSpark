 ...
 */
public class PartialQueryNativeMultiThread implements PairFlatMapFunction<Iterator<HashMultiMapNative>, Long, List<MatchCandidate>> {

    private static final Log LOG = LogFactory.getLog(PartialQueryNativeMultiThread.class);
    ...
    public PartialQueryNativeMultiThread(String file_name, long init, int bufferSize//) {
        ...
        try{
            ...
            while(myHashMaps.hasNext()){
                ...
                ThreadPoolExecutor executorPool = new ThreadPoolExecutor(this.options.getNumThreads(),
                        this.options.getNumThreads(), 10, TimeUnit.SECONDS, new ArrayBlockingQueue<Runnable>(this.options.getNumThreads()));
                ...
                while((seqReader.next() != null) && (currentSequence < (this.init + this.bufferSize))) {
                    ...
                    executorPool.execute(new RunnableClassification("Thread" + current_thread + "Seq"+currentSequence,
                            header, data, qua, this.options, this.window_stride, finalResults, currentHashMap, currentSequence));
    ...
    class RunnableClassification implements Runnable {
        ...
        RunnableClassification(String name, String header, String data, String qua, MetaCacheOptions options,
